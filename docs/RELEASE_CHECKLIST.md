...
- [x] Chrome 冒烟覆盖多题页面 Alt+双击 DOM→JEV、截图→视觉模型成功识别→JEV、视觉模型不支持时→本地 OCR→普通文本结构化→JEV、流式答案解析；并确认仅本地路径不会上传视觉截图、禁用站点不发起请求、会话清理后无残留
...
- [x] 没有 TypeSafe Key 时，已确认的文本题可由用户显式触发普通模型直答；不绕过结构、视觉语义或人工校正门禁
