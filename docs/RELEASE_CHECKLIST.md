...
- [x] Chrome 冒烟覆盖多题页面 Alt+双击 DOM→JEV（在测试环境授予可选网页权限时）、视觉模型→直接答案→按需详情识别、视觉模型不支持时→本地 OCR→普通文本结构化→JEV、无 JEV 时 OCR 原文→普通模型答案→按需详情识别、流式答案解析；并确认仅本地路径不会上传视觉截图、禁用站点不发起请求、浏览器重启后恢复本机配置与 API Key、显式清除 API Key 后无残留。无头 Chrome 拒绝可选网页权限时，Alt+双击部分保留到人工验收。
...
- [x] 结果详情可由用户显式触发普通模型直答；没有 TypeSafe Key 时 DOM 题自动直答、OCR 题整体直答并在详情中补做分离；不绕过视觉语义或人工校正门禁
