...
- [x] Chrome 冒烟覆盖多题页面 Alt+双击 DOM→JEV、截图→视觉模型成功识别→JEV、视觉模型不支持时→本地 OCR→普通文本结构化→JEV、流式答案解析；并确认仅本地路径不会上传视觉截图、禁用站点不发起请求、浏览器重启后恢复本机配置与 API Key、显式清除 API Key 后无残留
