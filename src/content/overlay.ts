...
export class ResultOverlay {
  ...
  private previewHtml() {
    ...
    return `<details class="preview" open><summary>识别原图、OCR 文本与文本框</summary><div class="preview-image"><img src="${escapeHtml(p.imageDataUrl)}" alt="本次识别的题目截图">${boxes}</div><pre class="ocr-text">${escapeHtml(rawText || "（未检测到文字）")}</pre><small>点击或悬停文本框可查看逐项 OCR 置信度；截图仅保留在本次覆盖层内。</small></details>`;
  }
...
function warningText(w: string) { return ({ LOW_OCR_CONFIDENCE: "OCR 置信度较低", POSSIBLE_FORMULA: "可能包含公式", POSSIBLE_DIAGRAM: "可能依赖图形", INCOMPLETE_OPTIONS: "选项可能不完整", VISION_MODEL_REQUIRED: "建议使用视觉模型", VISION_SERVICE_UNAVAILABLE: "视觉服务暂时不可用，已降级到本地 OCR", DOM_OCR_CONFLICT: "网页文字与 OCR 结果冲突" } as Record<string, string>)[w] ?? w; }
