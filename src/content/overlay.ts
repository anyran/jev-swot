...
export function summarizeAnswer(probability: ProbabilityResult): AnswerSummary {
  ...
  if (probability.mode === "single-distribution") {
    const top = sorted[0];
    return { label: top?.label ?? "待确认", uncertain: probability.confidence != null && probability.confidence < 0.6, detail: top ? `${(top.probability * 100).toFixed(0)}%` : "" };
  }
