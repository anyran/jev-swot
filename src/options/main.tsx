...
function App() {
  ...
  return <main><h1>Jev 做题家设置 <small>Jev SWOT</small></h1><p className="lead">模型地址、模型名和 API Key 会保存在本机 Chrome 配置中；浏览器重启后仍会保留。“清除本次会话”不会删除已保存配置；“清除 API Key”会主动删除会话和本机保存的密钥。</p>{shortcutMissing && <p className="message warning">框选快捷键未注册，可能与其他扩展冲突。请打开 <code>chrome://extensions/shortcuts</code> 手动设置。</p>}{!pageAccessGranted && <p className="message warning">尚未授予网页访问权限。扩展按钮和框选快捷键仍可在用户主动触发后临时使用；如需直接使用 Alt + 双击，请点击下方按钮授权任意 HTTP/HTTPS 网页。</p>}<form onSubmit={save}>
    ...
    <div className="actions">{!pageAccessGranted && <button type="button" className="muted" onClick={enablePageAccess}>启用任意网页双击识别</button>}<button type="button" className="muted" onClick={releaseOcr}>释放 OCR 内存</button><button type="button" className="muted" onClick={clearSession}>清除本次会话</button><button type="button" className="muted" onClick={clearApiKeys}>清除 API Key</button><button type="button" className="muted" onClick={testConnections}>测试连接与能力</button><button type="submit">保存设置</button></div>{saved && <p className="message">{saved}</p>}
