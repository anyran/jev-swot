...
function App() {
  ...
  async function save(event: React.FormEvent) {
    ...
    let origin: string;
    try { origin = new URL(settings.llm.baseUrl).origin + "/*"; }
    catch { setSaved("Base URL 不是有效的网址。"); return; }
  ...
  return <main><h1>JevAnswer 设置</h1><p className="lead">密钥仅保存在 Chrome 会话存储中。浏览器重启后需要重新填写。</p><form onSubmit={save}>
    ...
    <section><h2>隐私</h2><label>禁用站点（每行一个域名）<textarea value={settings.disabledHosts.join("\n")} onChange={e => updateSettings({ ...settings, disabledHosts: e.target.value.split(/\n+/).map(x => x.trim().toLowerCase()).filter(Boolean) })}/></label></section>
    <div className="actions"><button type="button" className="muted" onClick={releaseOcr}>释放 OCR 内存</button><button type="button" className="muted" onClick={clear}>清除会话密钥</button><button type="submit">保存设置</button></div>{saved && <p className="message">{saved}</p>}
