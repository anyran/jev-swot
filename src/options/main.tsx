...
function App() {
  ...
  async function clear() { await chrome.runtime.sendMessage({ type: "CLEAR_SESSION" }); updateSecrets({}); setSaved("已清除会话和本机保存的 API Key。"); }
  ...
  return <main><h1>Jev 做题家设置 <small>Jev SWOT</small></h1><p className="lead">模型地址、模型名、能力设置和 API Key 会保存在本机 Chrome 配置中；浏览器重启后仍会保留。点击“清除 API Key”可主动删除保存的密钥。</p>{shortcutMissing && <p className="message warning">框选快捷键未注册，可能与其他扩展冲突。请打开 <code>chrome://extensions/shortcuts</code> 手动设置。</p>}<form onSubmit={save}>
    ...
    <div className="actions"><button type="button" className="muted" onClick={releaseOcr}>释放 OCR 内存</button><button type="button" className="muted" onClick={clear}>清除 API Key</button><button type="button" className="muted" onClick={testConnections}>测试连接与能力</button><button type="submit">保存设置</button></div>{saved && <p className="message">{saved}</p>}
