...
function App() {
  ...
  async function testConnections() {
    await Promise.all([setSettings(settings), setSecrets(secrets)]); setSaved("正在测试连接…");
  ...
  return <main><h1>JevAnswer 设置</h1><p className="lead">密钥仅保存在 Chrome 会话存储中。浏览器重启后需要重新填写。</p>{shortcutMissing && <p className="message warning">框选快捷键未注册，可能与其他扩展冲突。请打开 <code>chrome://extensions/shortcuts</code> 手动设置。</p>}<form onSubmit={save}>
